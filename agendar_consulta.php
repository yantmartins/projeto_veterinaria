...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($clinica->cadastrarAgendamento($pet_id, $data, $hora, $descricao)) {
...
<body>
    ...
    <div class="container">
        <h2>Formulário de Agendamento de Consulta</h2>
        ...
        <form method="POST" action="agendar_consulta.php">
            <select name="pet_id" required>
                ...
                <?php
                $clinica = new ClinicaVeterinaria();
                $pets = $clinica->listarPets();
                foreach ($pets as $pet) {
                    echo "<option value='" . $pet['id'] . "'>" . $pet['nome'] . " (" . $pet['dono'] . ")</option>";
                }
                ?>
            </select>
            <input type="date" name="data" required>
            <input type="time" name="hora" required>
            <textarea name="descricao" placeholder="Descrição do motivo da consulta" required></textarea>
